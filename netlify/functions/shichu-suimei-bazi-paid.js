// netlify/functions/shichu-suimei-bazi-paid.js
// 四柱推命・¥500鑑定のエンドポイント。
...
const { calculateMeishiki, getYearPillar, GOGYO_BY_KAN, GOGYO_BY_SHI } = require('../../lib/shichu-suimei/bazi.js');
const { KAN_PERSONALITY } = require('../../lib/shichu-suimei/kan-personality.js');
const { getYearFortune } = require('../../lib/shichu-suimei/gogyo-relation.js');
...
exports.handler = async (event, context) => {
  ...
  if (!isHourUnknown && (isNaN(h) || h < 0 || h > 23)) {
    return { statusCode: 400, body: JSON.stringify({ error: "出生時刻が正しくありません。" }) };
  }
  ...
  try {
    ...
    return { statusCode: 500, body: JSON.stringify({ error: "計算中にエラーが発生しました。生年月日・時刻をご確認ください。" }) };
  }
};
